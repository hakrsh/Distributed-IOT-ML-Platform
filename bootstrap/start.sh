...
#####################
#making master password less
sshpass -p $pass ssh -o 'StrictHostKeyChecking no' $user "mkdir -p .ssh"
cat ~/.ssh/id_rsa.pub | sshpass -p $pass ssh $user  'cat >> .ssh/authorized_keys'
# Installing necessary scripts
ssh $user 'sudo apt-get update'
# Installing python3
ssh $user 'sudo apt-get install python3 -y'
# Installing pip3
ssh $user 'sudo apt-get install -y python3-pip'
# Installing pip3-docker
ssh $user 'pip3 install docker'
#Installing docker on Master
ssh $user 'wget https://gist.githubusercontent.com/097nitinkumar/f2ab08718f489df02f8b23f1e922e543/raw/ecec210efaa6eff98a0091e521e6daf8f06f08df/install-docker.sh'
ssh $user 'chmod +x install-docker.sh'
ssh $user './install-docker.sh'
#Installing java for kafka
# ssh $master 'sudo apt-get install default-jre -y'
ssh $user 'sudo apt-get install default-jre -y'
# Installing kafka
...
# Note: We had to update config file also, in kafka (below)
############uncomment below code later###############
ssh $user 'cat ~/kafka_2.13-3.1.0/config/server.properties'
...
# #######################Run at end#########################
#Installing haproxy
ssh $user 'sudo apt show haproxy'
ssh $user 'sudo apt update'
ssh $user 'sudo apt install -y haproxy'
# #Note: We had to update config file also, in haproxy
##############################################uncomment below code later###############
ssh $user 'sudo chmod 777 /etc/haproxy/haproxy.cfg'
ssh $user 'cat /etc/haproxy/haproxy.cfg'
